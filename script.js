...
const oldInventors = inventors.filter((inventors) => {
  return inventors.year > 1500 && inventors.year < 1599;
});

veryOldInventors.push(oldInventors);
...
const birthYearAsc = inventors.sort((a, b) => a.year - b.year);

sortedByBirthYear.push(birthYearAsc);
...
const peopleFirstLast = people.map((personName) => {
  return personName.split(", ").reverse().join(" ");
});
// personName represents each string element in the 'people' array
// split(', ) - divides the string into an array of substrings e.g. ['Becker, Carl', 'Beckett, Samuel'] -> ['Becker', 'Carl', 'Beckett', 'Samuel']
// reverse() - reverses the order of elements in the substring
// join() - concatenates all elements in the reversed array into a single string with ' ' seperator

// this method is destructive? - reverses the split array

firstLast.push(peopleFirstLast);
...
isEveryone19OrOlder = devs.every((every19) => {
  return 2024 - every19.year >= 19;
});
...
commentById = comments.find((obj) => {
  return obj.id === 823423;
});
...
idx = comments.findIndex((commentIndex) => {
...
travelMethodCounts = travelMethods.reduce((accumulator, method) => {
...
console.log("Bonus 2 Correct Result: ", {
  car: 5,
  truck: 3,
  bike: 2,
  walk: 2,
